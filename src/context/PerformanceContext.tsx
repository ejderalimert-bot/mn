...
export function PerformanceProvider({ children, defaultMode }: { children: React.ReactNode, defaultMode?: PerformanceMode }) {
  ...
  const setPerformanceMode = (mode: PerformanceMode) => {
    ...
    if (dontShowAgain && mode) {
      document.cookie = `stardublajweb_perf_v2=${mode}; path=/; max-age=31536000`; // 1 year cookie
    }
  };
        ...
        {performanceMode === null && (
          ...
          >
            <motion.div
              ...
              animate={{ scale: 1, y: 0 }}
              className="bg-[#0b0c10] border border-stardublajweb-purple/30 rounded-[2rem] p-8 md:p-12 shadow-[0_0_100px_rgba(168,85,247,0.3)] max-w-2xl w-full text-center relative overflow-hidden"
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative z-10">
                <button
                  ...
                  className="group relative bg-[#1a1c23] border border-stardublajweb-purple/50 rounded-2xl p-8 hover:bg-stardublajweb-purple/10 transition-all text-left flex flex-col items-center justify-center overflow-hidden h-64"
                >
                  ...
                  <Zap className="w-16 h-16 text-stardublajweb-purple mb-6 group-hover:scale-125 transition-transform" />
                  <h3 className="text-xl font-black text-white uppercase text-center">SİNEMATİK MOD</h3>
                  <p className="text-stardublajweb-purple text-xs font-bold mt-2 text-center">YÜKSEK PERFORMANSLI PC'LER</p>
