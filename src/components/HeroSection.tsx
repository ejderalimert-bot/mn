  ...
  return (
    <div onMouseMove={handleMouseMove} className="relative min-h-[110vh] pb-32 pt-20 w-full flex flex-col items-center justify-center overflow-hidden bg-black [perspective:1000px]">
      ...
      <motion.div style={{ y: yBg }} className="absolute inset-0 pointer-events-none z-0">
        <motion.div animate={{ rotate: 360, scale: [1, 1.5, 1] }} transition={{ repeat: Infinity, duration: 10, ease: "linear" }} className="absolute top-[-20%] left-[-10%] w-[1200px] h-[1200px] bg-stardublajweb-cyan/20 blur-[250px] mix-blend-screen rounded-full" />
        <motion.div animate={{ rotate: -360, scale: [1.5, 1, 1.5] }} transition={{ repeat: Infinity, duration: 8, ease: "linear" }} className="absolute bottom-[-30%] right-[-10%] w-[1500px] h-[1500px] bg-stardublajweb-purple/30 blur-[300px] mix-blend-screen rounded-full" />
      ...
      <motion.div style={{ y: yBg }} className="absolute inset-0 z-0 [transform-style:preserve-3d]">
        <motion.div animate={{ rotateX: 360, rotateY: 360, rotateZ: 360 }} transition={{ repeat: Infinity, duration: 4, ease: "linear" }} className="absolute top-[20%] left-[10%] w-48 h-48 border-[4px] border-stardublajweb-cyan/40 shadow-[0_0_100px_cyan] rounded-[3rem] bg-cyan-500/10 backdrop-blur-3xl" />
        <motion.div animate={{ rotateX: -360, rotateY: 360, scale: [1, 1.5, 1] }} transition={{ repeat: Infinity, duration: 6, ease: "linear" }} className="absolute bottom-[20%] right-[15%] w-64 h-64 border-[4px] border-stardublajweb-purple/40 shadow-[0_0_150px_purple] rounded-full bg-purple-500/10 backdrop-blur-3xl flex items-center justify-center"><Gamepad2 className="w-32 h-32 text-stardublajweb-purple opacity-50" /></motion.div>
      ...
      <motion.div style={{ y: yText, rotateX: 10 }} className="container relative z-20 flex flex-col items-center text-center [transform-style:preserve-3d] mt-10 mb-20">
        <motion.div animate={{ rotateY: 360 }} transition={{ repeat: Infinity, duration: 10, ease: "linear" }} className="inline-flex items-center gap-3 px-8 py-3 bg-black/50 border-[2px] border-stardublajweb-cyan/50 rounded-full mb-12 shadow-[0_0_50px_cyan] backdrop-blur-xl">
          <Sparkles className="w-6 h-6 text-stardublajweb-cyan animate-spin" />
          <span className="text-lg font-black text-white tracking-[0.3em] uppercase drop-shadow-[0_0_10px_white]">Star Dublaj Stüdyo</span>
        </motion.div>
        <motion.h1 animate={{ scale: [1, 1.05, 1] }} transition={{ repeat: Infinity, duration: 2, ease: "easeInOut" }} className="text-[5rem] sm:text-[6rem] md:text-[9rem] font-black text-transparent bg-clip-text bg-gradient-to-br from-white via-stardublajweb-cyan to-stardublajweb-purple leading-[0.9] tracking-tighter mb-8 drop-shadow-[0_20px_100px_rgba(106,255,235,0.8)] [transform:translateZ(100px)] py-4">
...
const CleanHero = () => {
  ...
  return (
    <div className="relative min-h-[90vh] w-full flex flex-col items-center justify-center overflow-hidden bg-[#0a0a0c] selection:bg-stardublajweb-cyan/30">
